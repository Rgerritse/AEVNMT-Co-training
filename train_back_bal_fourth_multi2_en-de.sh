...
do
	session="cond_back_balanced_fourth_en-de_run_$((GPU+4))"

	CUDA_VISIBLE_DEVICES=${GPU} python -u ${HOME}/code/coaevnmt/train_super.py \
		--session "${session}" \
		--config ${HOME}/code/coaevnmt/config/cond_nmt_multi30k_en-de.json \
		--data_dir ${TMPDIR}/data/multi30k \
		--out_dir ${OUTPUT_DIR} \
		--train_prefix "training_fourth" \
		--back_prefix "back_fourth_en-de_$((GPU+4))" \
