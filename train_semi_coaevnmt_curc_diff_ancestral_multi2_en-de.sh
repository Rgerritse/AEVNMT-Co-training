...
do
	session="coaevnmt_curc_diff_ancestral_en-de_run_$((GPU+4))"

	CUDA_VISIBLE_DEVICES=${GPU} python -u ${HOME}/code/coaevnmt/train_semi.py \
		--session "${session}" \
		--config ${HOME}/code/coaevnmt/config/coaevnmt_multi30k_curc_diff_ancestral_en-de.json \
