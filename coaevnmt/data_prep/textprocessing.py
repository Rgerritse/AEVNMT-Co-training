...
class WordDesegmenter(TextProcess):
    ...
    def __call__(self, line: str) -> str:
        return re.sub("({} )|({} ?$)|( {})|(^ ?{})".format(self.separator, self.separator, self.separator, self.separator), "", line)
        # return re.sub(f"({self.separ):ator} )|({self.separator} ?$)|( {self.separator})|(^ ?{self.separator})", "", line)
