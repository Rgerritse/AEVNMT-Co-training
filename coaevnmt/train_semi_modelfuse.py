...
def create_model(vocab_src, vocab_tgt, config):
    ...
    if config["model_type"] == "conmt":
        ...
        model = CoAEVNMT(vocab_src, vocab_tgt, config)
    ...
    return model, bi_train_fn, mono_train_fn, validate_fn
...
def train(model, bi_train_fn, mono_train_fn, validate_fn, bucketing_dl_xy,
    ...
    optimizers_xy, schedulers_xy = create_optimizers(
        model.model_xy.generative_parameters(),
        model.model_xy.inference_parameters(),
        config
    )

    optimizers_yx, schedulers_yx = create_optimizers(
        model.model_yx.generative_parameters(),
        model.model_yx.inference_parameters(),
        config
    )
    ...
    asf
        # init_model(model_xy, vocab_src[PAD_TOKEN], vocab_tgt[PAD_TOKEN], conf/ig)
        # init_model(model_yx, vocab_tgt[PAD_TOKEN], vocab_src[PAD_TOKEN], config)
...
def main():
    ...
    model, bi_train_fn, mono_train_fn, validate_fn = create_model(vocab_src, vocab_tgt, config)
