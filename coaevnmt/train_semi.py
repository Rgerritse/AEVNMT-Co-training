...
def create_model(vocab_src, vocab_tgt, config):
    if config["model_type"] == "coaevnmt":
        ...
        train_fn_bilingual = coaevnmt_utils.train_step_bilingual
        validate_fn = coaevnmt_utils.validate
    else:
        raise ValueError("Unknown model type: {}".format(config["model_type"]))
    print("Model: ", model)
    return model, train_fn_bilingual, validate_fn
...
def train(model, train_fn, validate_fn, dataloader, dataset_dev, src_mono_buck, tgt_mono_buck, vocab_src, vocab_tgt, config):
    ...
    for epoch in range(saved_epoch, config["num_epochs"]):
        for step, batch in enumerate(dataloader):
            ...
            loss = train_fn_bilingual(model, prev_x, x, x_mask, prev_y, y, y_mask, src_pad_idx, tgt_pad_idx, step)
            # Monolingual
            ...
            # print("x: ", vocab_src.array_train_semi.py --session semi --config config/coaevnmt.json to_sentence(x[-1].cpu().numpy(), cut_at_eos=False))
            ...
            loss = train_fn(model, prev_x, x, x_mask, prev_y, y, y_mask, prev_y_mono, y_mono, y_mono_mask, prev_x_mono, x_mono, x_mono_mask, config["share_latent_var"], src_pad_idx, tgt_pad_idx, step)
            loss.backward()
...
def main():
    ...
    model, train_fn_bilingual, validate_fn = create_model(vocab_src, vocab_tgt, config)
    model.to(torch.device(config["device"]))

    init_model(model, vocab_src.stoi[config["pad"]], vocab_tgt.stoi[config["pad"]], config)
    train(model, train_fn_bilingual, validate_fn, dataloader,
