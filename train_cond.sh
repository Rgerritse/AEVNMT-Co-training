...
session="cond_nmt-new_loss"



OUTPUT_DIR="${HOME}/code/output/${session}"
mkdir ${OUTPUT_DIR}

python -u ${HOME}/code/main.py \
	--session "${session}" \
	--config ${HOME}/code/config/cond_nmt.json \
	--data_dir ${TMPDIR}/data/setimes.tokenized.en-tr \
	--out_dir ${OUTPUT_DIR} \
	&> ${HOME}/code/output/${session}/log_file
