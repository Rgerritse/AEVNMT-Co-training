...
do
	session="coaevnmt_lr3_beam_dec_5_en-de_run_${GPU}"

	CUDA_VISIBLE_DEVICES=${GPU} python -u ${HOME}/code/coaevnmt/train_semi.py \
		--session "${session}" \
		--config ${HOME}/code/coaevnmt/config/coaevnmt_multi30k_lr3_en-de.json \
		--data_dir ${TMPDIR}/data/multi30k \
		--out_dir ${OUTPUT_DIR} \
		--decoding_method beam_search \
		--decoding_beam_width 5 \
