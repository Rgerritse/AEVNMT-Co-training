...
do
	session="coaevnmt_lr3_no_curriculum_no_warmup_en-de_run_${GPU}"

	CUDA_VISIBLE_DEVICES=${GPU} python -u ${HOME}/code/coaevnmt/train_semi.py \
		--session "${session}" \
		--config ${HOME}/code/coaevnmt/config/coaevnmt_multi30k_lr3_no_warmup_en-de.json \
