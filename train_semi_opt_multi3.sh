#!/bin/bash
#SBATCH --nodes=1
#SBATCH --job-name=semi_g_off_train
...
do
	session="coaevnmt_opt3_ada_run_${GPU}"

	CUDA_VISIBLE_DEVICES=${GPU} python -u ${HOME}/code/coaevnmt/train_semi_new2.py \
		--session "${session}" \
		--config ${HOME}/code/coaevnmt/config/coaevnmt_multi30k_opt3.json \
